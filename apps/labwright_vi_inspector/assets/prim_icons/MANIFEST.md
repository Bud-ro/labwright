...
Class-identified growable nodes (`class<code>_t<arity>`) are deliberately
NOT bundled: every extraction reviewed so far baked neighbour state (wires,
...
| prim1052_multiply.png | Multiply | 21x21 | full-node-box crop from Excel_Read_XLSX (wire ink erased), trimmed to its ink bbox; byte-verified against the reference at its node |
...
| prim1110_greater.png | Greater? | 22x21 | example — single-sample fallback (0/1 agreeing) |
| prim1112_empty-string-path.png | Empty String/Path? | 21x21 | full-node-box crop from Excel_Read_XLSX (wire ink erased), trimmed to its ink bbox; byte-verified against the reference at its node |
| prim1114_greater-or-equal-to-0.png | Greater Or Equal To 0? | 32x29 | Config_Load2, Config_Load, ClassChildren, Read Library Version — single-sample fallback after edge fusion (2/4 agreeing) |
...
| prim1503_string-subset.png | String Subset | 32x32 | border-exact rect (2/2 ring samples agree byte-for-byte): Config_Load2, Config_Load; the baked wire stub above the top border erased |
| prim1435.png | (uncatalogued) | 23x18 | full-node-box crop from Excel_Read_XLSX (wire ink erased), trimmed to its ink bbox; byte-verified against the reference at its node |
...
| prim1907_array-max-min.png | Array Max & Min | 37x32 | border-exact rect (1/1 ring samples agree byte-for-byte): ClassChildren |
...
| prim2073_create-user-event.png | Create User Event | 37x32 | border-exact rect (2/2 ring samples agree byte-for-byte): Pages |
| prim2074_generate-user-event.png | Generate User Event | 35x32 | border-exact rect (2/2 ring samples agree byte-for-byte): Pages, Page1 |
| prim2302.png | (uncatalogued) | 32x32 | border-exact rect (1/1 ring samples agree byte-for-byte): VISA_Open2 |
| prim23063_empty-array.png | Empty Array? | 38x27 | Excel_Variant_Elements, Export Palette Image WMF, ClassChildren, Excel_Cell_to_Value, GenerateTree, large — single-sample fallback (0/8 agreeing) |
...
| prim2457.png | (uncatalogued) | 36x32 | border-exact rect (1/1 ring samples agree byte-for-byte): GenerateTree |
| prim2458.png | (uncatalogued) | 33x32 | border-exact rect (1/1 ring samples agree byte-for-byte): GenerateTree |
| prim8010_open-vi-reference.png | Open VI Reference | 32x32 | border-exact rect (1/1 ring samples agree byte-for-byte): Pages |
| prim8011_close-reference.png | Close Reference | 37x32 | border-exact rect (2/2 ring samples agree byte-for-byte): Resolve Library Path |
...
| prim8073_set-file-position.png | Set File Position | 17x32 | border-exact rect (1/1 ring samples agree byte-for-byte): Read Library Version |
...
| prim8203_variant-to-flattened-string.png | Variant To Flattened String | 32x23 | border-exact rect (1/1 ring samples agree byte-for-byte): Excel_Variant_Elements |
