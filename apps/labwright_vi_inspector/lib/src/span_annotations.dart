...
SpanInfo classifySpan(Uint8List bytes, HeapSpan span, String tag) {
  ...
  SpanInfo make(Color color, String title, String detail, {Color? swatch, Widget? display, String? inlinePreview}) =>
      SpanInfo(
          offset: offset,
          length: len,
          lead: lead,
  ...
  if ((lead == 0x10 || lead == 0x11 || lead == 0x12) && offset + 9 <= bytes.length && bytes[offset + 2] == 0x02 && bytes[offset + 3] == 0xfe && bytes[offset + 6] == 0xfd) {
    final kind = readU16be(bytes, offset + 4), oid = readU16be(bytes, offset + 7);
    final cls = HeapObjectClass.fromCode(kind);
    final conf = cls.confidence == ClassConfidence.confirmed ? '' : ' (${cls.confidence.name})';
    return make(spanColorObject, 'Object · ${cls.label}',
  ...
  if (prop != null) {
    ...
    final conf = token.confidence == AttrConfidence.confirmed ? '' : ' (${token.confidence.name})';
    final hexpair = '${lead.toRadixString(16).padLeft(2, '0')} ${bytes[offset + 1].toRadixString(16).padLeft(2, '0')}';
    final val = prop.value == null ? '' : ' = ${prop.value}';
    return make(spanColorAttr, '$hexpair · ${token.tokenName}', 'Object property$val$conf.');
  }
  String _hx(int value) => '0x${value.toRadixString(16).padLeft(2, '0')}';
  if (lead == 0x10 || lead == 0x11 || lead == 0x12 || lead == 0x13) {
    final hasTypeTag = offset + 3 < bytes.length && (bytes[offset + 3] == 0xfb || bytes[offset + 3] == 0xfe || bytes[offset + 3] == 0xfd);
    if (hasTypeTag) {
      final tag = offset + 1 < bytes.length ? bytes[offset + 1] : -1;
      return make(spanColorGroup, 'Group open · tag ${_hx(tag)}',
  ...
  if (lead == 0x08 || lead == 0x09 || lead == 0x0a || lead == 0x0b) {
    final tag = offset + 1 < bytes.length ? bytes[offset + 1] : -1;
    return make(spanColorGroup, 'Group close · tag ${_hx(tag)}',
  ...
  if (ref != null) {
    ...
    return make(spanColorRef, '${ref.kind.refName} → #${ref.targetOid}',
  ...
  if (lead == kHeapRecordPrefix) {
    ...
    if (rec != null) {
      ...
      final rect = rec.rect;
      if (rect != null) {
        return make(spanColorRect, '$hexop · ${opcode.name}',
      ...
      if (text != null && text.isNotEmpty) {
        return make(spanColorString, '$hexop · ${opcode.name}', 'String payload:', display: StringPreview(text));
      }
      final isContainer = opcode == HeapOpcode.container24 || opcode == HeapOpcode.container44 || opcode == HeapOpcode.container64;
      return make(isContainer ? spanColorContainer : spanColorOther, '$hexop · ${opcode == HeapOpcode.unknown ? 'record' : opcode.name}',
          opcode.isDecoded ? 'A decoded ${opcode.name} record.' : 'A framed ${opcode.name} record (${rec.payload.length}-byte payload).');
    }
  }
  ...
  if (attr != null) {
    ...
    switch (attr.kind) {
      case HeapAttrKind.color:
        ...
        return make(spanColorAttr, '$hexlead · $name',
            attr.isTransparent ? 'Colour: transparent.' : 'Colour #${rgb.toRadixString(16).padLeft(6, '0')}.',
            swatch: swatch, display: swatch == null ? null : ColorPreview(swatch, rgb));
      case HeapAttrKind.controlParam:
        return make(spanColorAttr, '$hexlead · $name', 'Numeric-control parameter (f64) = ${attr.asDouble}.');
      case HeapAttrKind.stringBlob:
        return make(spanColorString, '$hexlead · $name', 'String/blob:', display: StringPreview(attr.asString ?? ''));
      case HeapAttrKind.rectangle:
        final rect = attr.asRect;
        return rect == null
            ? make(spanColorAttr, '$hexlead · $name', 'Rectangle (4× s16).')
            : make(spanColorRect, '$hexlead · $name',
                'Rectangle (4× s16): top ${rect.top}, left ${rect.left}, bottom ${rect.bottom}, right ${rect.right}  (${rect.width}×${rect.height}).',
                display: RectPreview(rect));
      case HeapAttrKind.container:
        return make(spanColorContainer, '$hexlead · $name',
  ...
  if (isTypeDescriptorToken(lead)) {
    return make(spanColorOther, '04 ${bytes[offset + 1].toRadixString(16).padLeft(2, '0')} · 04-token',
        'A bare 04 <subop> token (framed; role undecoded).');
  }
  return make(spanColorOther, 'lead 0x${lead.toRadixString(16)}', 'Framed record ($len bytes); role not individually decoded.');
}
...
class RectPreview extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      width: width,
      height: height,
      decoration: BoxDecoration(border: Border.all(color: spanColorRect), color: spanColorRect.withValues(alpha: 0.15)),
...
class ViIconImage extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) =>
      SizedBox(width: size, height: size, child: CustomPaint(painter: RgbIconPainter(icon)));
}
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var y = 0; y < icon.height; y++) {
      for (var x = 0; x < icon.width; x++) {
        paint.color = Color.fromARGB(255, icon.rgb[k], icon.rgb[k + 1], icon.rgb[k + 2]);
        k += 3;
        canvas.drawRect(Rect.fromLTWH(x * pw, y * ph, pw + 0.5, ph + 0.5), paint);
      }
    }
  }
