    ...
    (tester) async {
      ...
      await tester.runAsync(() async {
        ...
        expect(
          inputPt,
          isNotNull,
          reason: 'the U8 op must ship an input route',
        );
            ...
            for (var y = attach.top - 2; y < attach.bottom + 2; y++) {
              for (var x = attach.left - 2; x < attach.right + 2; x++) {
                ...
                if (inside) {
                  ...
                  bandCompared++;
                  if (refAt(x, y) == canvasWhite &&
                      oursAt(x, y) != canvasWhite) {
                    bleed++;
                  }
                }
              }
        ...
        for (final sig in [403, 1831, 921]) {
          ...
          expect(
            w.routePoints,
            isNotNull,
            reason: 'sig $sig must ship a route',
          );
          expect(
        ...
        // ignore: avoid_print
        print(
          'oid1988 wire mask: $maskCount px, $maskMismatched off reference',
        );
        expect(
          maskCount,
          greaterThan(400),
          reason: 'the wire draws a long run',
        );
        expect(
          maskMismatched,
          0,
        ...
        for (final (sig, value) in [(375, 8), (3126, 256), (399, 256)]) {
          ...
          expect(
            ...
            span,
            reason:
                'the reference wires the $value constant box to its terminal',
          );
          expect(
            rasterInkSpan,
            span,
            reason:
                'the render must route the $value feeder to the constant box',
          );
        }
      });
    },
