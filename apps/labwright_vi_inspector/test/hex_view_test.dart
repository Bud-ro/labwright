...
void main() {
  ...
  final cases = <HexCase>[
    ...
    HexCase(
      'a vers block frames its layout rows',
      raw('vers', versBytes),
      one: [
        'versionWord (u32)',
        'flags (u16)',
    ...
    HexCase(
      'an LVSR block frames its decoded rows and marks the rest undecoded',
      raw(
        'LVSR',
      ...
      one: [
        'versionWord (u32)',
        'saveFlags (u16)',
        'blockDiagramPasswordDigest (u8[16])',
        'secondaryDigest (u8[16])',
        '% framed',
      ],
      some: ['Undecoded'],
      never: ['100% framed'],
    ...
    HexCase(
      'a NUID id table frames count + ids (100%)',
      raw('NUID', nuidBytes),
      one: ['count (u32)', 'ids (u32[count])', '100% framed'],
      never: ['Undecoded'],
    ),
    HexCase(
      'an FPSE section entry is framed as a u32 (100%)',
      raw('FPSE', [0, 0, 0, 0x77]),
      one: ['value (u32)', '100% framed'],
      never: ['Undecoded'],
    ),
    HexCase(
    ...
    HexCase(
      'a TITL block is a Pascal string (100%)',
      raw('TITL', [11, ...'Batch Tests'.codeUnits]),
      one: ['title (pstr)', '100% framed'],
      some: ['Batch Tests'],
      never: ['Undecoded'],
    ),
    HexCase(
      'an FTAB block frames its header and the record run',
      raw('FTAB', [
        0,
        1,
        0,
      ...
      one: ['version (u16)', 'fontCount (u16)', '% framed'],
      some: ['Undecoded'],
    ),
    HexCase(
      'a CPSP string table is framed (count + strings, 100%)',
      raw('CPSP', cpsBytes),
      one: ['count (u32)', 'strings (pstr[count])', '100% framed'],
      never: ['Undecoded'],
    ),
    HexCase(
      'a CPST string table is framed the same way',
      raw('CPST', cpsBytes),
      one: ['count (u32)', 'strings (pstr[count])', '100% framed'],
      never: ['Undecoded'],
    ),
    HexCase(
      'an FPTD block is framed as its u16 words (100%)',
      raw('FPTD', [0x01, 0x52]),
      one: ['words (u16[])', '100% framed'],
    ),
    HexCase(
