...
void main() {
  test('run: a directory resolves to its main.dart; flags become defines;'
    ...
    try {
      ...
      expect(result.exitCode, 1,
          reason: 'the suite contains failures:\n${result.stdout}');
      final report = (jsonDecode(File(reportPath).readAsStringSync()) as Map)
          .cast<String, Object?>();
      final tests = (report['tests'] as List).cast<Map<String, Object?>>();
      expect(tests.map((t) => t['name']).toSet(), _allTests,
  ...
  test('run + shards + seed: the shards partition the suite, none missed',
    ...
    for (final seedArg in ['0', '7']) {
      ...
      for (var index = 0; index < 2; index++) {
        ...
        try {
          ...
          expect(result.exitCode, anyOf(0, 1),
              reason: 'shard $index seed $seedArg crashed:\n'
                  '${result.stdout}\n${result.stderr}');
          final report =
              (jsonDecode(File(reportPath).readAsStringSync()) as Map)
                  .cast<String, Object?>();
          expect(report['seed'], int.parse(seedArg));
          for (final t
              in (report['tests'] as List).cast<Map<String, Object?>>()) {
      ...
      expect(executed, hasLength(_allTests.length),
          reason: 'seed $seedArg: every test exactly once across shards');
      expect(executed.toSet(), _allTests,
          reason: 'seed $seedArg: union of shards is the whole suite');
    }
  ...
  test('scan: names unplugged files and exits 1; clean suite exits 0', () {
    ...
    expect(dirty.stdout.toString(), contains('unplugged.dart'),
        reason: 'the forgotten file is named');
    // Path-anchored: "unplugged.dart" itself contains "plugged.dart".
    expect(dirty.stdout.toString(),
  ...
  test('scan is AST-driven: outside-folder plumbing, exports, renames, '
    ...
    expect(out, contains('ghost.dart'),
        reason: 'mentions of ghost.dart in comments and string literals are '
            'not directives — a text-match walk would have missed it');
    // ...and none of the tricky-but-plugged ones are:
    expect(out, isNot(contains('via_outside.dart')),
        reason: 'plugged THROUGH a helper outside the scanned folder');
    expect(out, isNot(contains('renamed_symbols.dart')),
        reason: 'reached via an export; renamed test symbols are irrelevant');
    expect(out, isNot(contains('decoy_mentions.dart')),
        reason: 'directly imported by main.dart');
    expect(out, isNot(contains('conditional_io.dart')),
        reason: 'the default branch of a conditional import is plugged');
    expect(out, isNot(contains('conditional_js.dart')),
        reason: 'the non-default branch is part of the program too');
  });
  ...
  test('the tricky fixture is a real suite: it runs green single-process',
    ...
    final out = result.stdout.toString();
    expect(out, contains('✓ plugged via an outside helper'));
    expect(out, contains('✓ renamed test symbol'),
        reason: 'lab.test registers like test — aliasing changes nothing');
    expect(out, contains('✓ tear-off registered test'));
    expect(out, contains('✓ conditional io branch'),
        reason: 'the VM loads the default conditional branch');
    expect(out,
        contains('labwright: 4 test(s) — 4 passed, 0 failed, 0 errors, '
            '0 skipped'),
  ...
  test('init: generates the example suite; it runs green; refuses overwrite',
    ...
    try {
      ...
      expect(
          File('$pkgRoot/$dir/power_rail_test.dart').existsSync(), isTrue);
      ...
      expect(run.stdout.toString(), contains('✓ rail comes up [REQ-1]'));
