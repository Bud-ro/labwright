...
/// The code units of the ASCII range, whose case mapping [lvToLowerCase] is
/// defined over. `A`..`Z` map to `a`..`z` and every other unit is unchanged.
...
/// The distance from an upper-case ASCII letter to its lower-case partner.
const int _kAsciiCaseDistance = 0x20;

/// LabVIEW's **To Lower Case** over the ASCII range: every `A`..`Z` code unit
/// of [text] becomes its lower-case partner and every other unit below 0x80 is
/// returned unchanged.
///
...
/// is Unicode's, which differs there — so the upper half has no established
/// result to return.
...
/// A [bits]-wide [value] written in **hexadecimal**, left-padded with `0` to at
/// least [width] digits.
///
/// A value needing more than [width] digits keeps all of them: the width is a
/// minimum field width, not a truncation. [value] is read as an unsigned
/// [bits]-wide pattern, so a narrow signed carrier's sign bit prints as the bit
/// it is.
