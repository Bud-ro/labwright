...
const Map<String, ViBlockInfo> _catalog = {
  ...
  'VICD': ViBlockInfo('VICD', 'VI compiled code', _cc, _cf, '16-byte envelope: u32 flags + 4CC arch (i386/wx64/...) + u32le code size; body is target machine code (recognized opaque, not disassembled). See decodeCompiledCode.', decoder: 'decodeCompiledCode'),
  ...
  'DSIM': ViBlockInfo('DSIM', 'Data-space image', _ds, _cf, 'Leading u32==0 (18654/18654) + u16 geometry words; 15620/18654 embed the colour-icon PNG (envelope decoded). Remaining header semantics not yet decoded. See decodeDataSpaceImage.', decoder: 'decodeDataSpaceImage'),
  ...
  'CPMp': ViBlockInfo('CPMp', 'Connector pane map', _cp, _cf, '[u16le terminalCount][count x u16le object index, 0xFFFF = unassigned] (3531/3531 exact). Maps conpane terminals to panel objects. See decodeConnectorPaneMap.', decoder: 'decodeConnectorPaneMap'),
  ...
  'PICC': ViBlockInfo('PICC', 'Icon picture record', _ic, _cf, '12 B = six u16be fields [id, 1, top, left, bottom, right] (3283/3283); icon placement rect (roles per corpus geometry). See decodeIconPlacement.', decoder: 'decodeIconPlacement'),
  'PICT': ViBlockInfo('PICT', 'Mac PICT image', _im, _cf, 'QuickDraw PICT v2: bounds rect + 00 11 02 FF version opcode; stream is standard PICT. See decodePictEnvelope.', decoder: 'decodePictEnvelope'),

  'MNGI': ViBlockInfo('MNGI', 'PNG image', _im, _cf, 'PNG (magic 89 50 4E 47; rare MNG variant): envelope + IHDR dimensions decoded; pixel data is standard PNG. See decodePngEnvelope.', decoder: 'decodePngEnvelope'),
  'WEMF': ViBlockInfo('WEMF', 'Windows enhanced metafile', _im, _cf, 'EMF: EMR_HEADER (le) + " EMF" signature @40; bounds decoded, stream is standard EMF. See decodeEmfEnvelope.', decoder: 'decodeEmfEnvelope'),

  'LIvi': ViBlockInfo('LIvi', 'Link info: VI', _li, _cf, 'u16 ver=1 + "LVIN" + u32 entry count + linkage entries (VILB/VICC/...); dependency names + PTH0 paths recovered; full entry grammar not yet decoded. See decodeLinkInfo.', decoder: 'decodeLinkInfo'),
  'LIfp': ViBlockInfo('LIfp', 'Link info: front panel', _li, _cf, 'u16 ver=1 + "FPHP" + u32 entry count + linkage entries (FPPI/TDCC/...); typedef/control names recovered. See decodeLinkInfo.', decoder: 'decodeLinkInfo'),
  'LIbd': ViBlockInfo('LIbd', 'Link info: block diagram', _li, _cf, 'u16 ver=1 + "BDHP" + u32 entry count + linkage entries (IUVI/...); sub-VI dependency names recovered. See decodeLinkInfo.', decoder: 'decodeLinkInfo'),
  'LIds': ViBlockInfo('LIds', 'Link info: data space', _li, _cf, 'u16 ver=1 + "VIDS" + u32 entry count + linkage entries. See decodeLinkInfo.', decoder: 'decodeLinkInfo'),
  ...
  'BDPW': ViBlockInfo('BDPW', 'Block-diagram password', _se, _cf, 'Three 16-byte MD5 digests (48 B, 7538/7539; legacy 32 B once): password hash (d41d8cd9... = MD5("") when unprotected) + two derived digests (derivation not re-derived). See decodePasswordRecord.', decoder: 'decodePasswordRecord'),

  'VINS': ViBlockInfo('VINS', 'Embedded sub-VIs', _ev, _cf, 'Nested RSRC VIs, each a complete parseable VI. See readEmbeddedVis.', decoder: 'readEmbeddedVis'),
  ...
  'VITS': ViBlockInfo('VITS', 'VI tag store', _nt, _cf, '[u32 count] + count x [u32 nameLen][name][u32 payloadLen][payload] (5219/7203 walk exactly; the rest carry a trailing variant, flagged walkComplete=false). See decodeTagStore.', decoder: 'decodeTagStore'),
  ...
  'MUID': ViBlockInfo('MUID', 'Modified UID', _id, _cf, '4-byte u32 id, varied per VI. See decodeModifiedUid.', decoder: 'decodeModifiedUid'),
  ...
  'RTSG': ViBlockInfo('RTSG', 'Run-time signature', _id, _cf, '16-byte signature, varied per VI (7582/7582): a single opaque identity digest; format decoded, derivation not. See decodeRuntimeSignature.', decoder: 'decodeRuntimeSignature'),
  ...
  'SCSR': ViBlockInfo('SCSR', 'Source signature', _id, _cf, '20 B: u32 marker 0x01000000 + 16-byte signature. See decodeScsrRecord.', decoder: 'decodeScsrRecord'),
  'GCPR': ViBlockInfo('GCPR', 'Generated-code property', _id, _cf, 'Fixed 13-byte record, constant (all-zero) across the corpus. See decodeGcprRecord.', decoder: 'decodeGcprRecord'),
  'GCDI': ViBlockInfo('GCDI', 'Generated-code debug info', _id, _tt, 'Compressed; mostly 9 B decompressed; format not yet decoded.', decoder: 'decodeGcdiRecord'),

  'FPSE': ViBlockInfo('FPSE', 'Front-panel section entry', _un, _cf, '4-byte u32 (FP section offset/size marker; 7535/7582), rarely 8 B with a second word. See decodeSectionMarker.', decoder: 'decodeSectionMarker'),
  'BDSE': ViBlockInfo('BDSE', 'Block-diagram section entry', _un, _cf, '4-byte u32 (BD section offset/size marker; 7535/7582), rarely 8 B with a second word. See decodeSectionMarker.', decoder: 'decodeSectionMarker'),
  'FPEx': ViBlockInfo('FPEx', 'Front-panel extended', _un, _cf, 'u32be flag words (4-32 B dominate); bit meanings not yet decoded. See decodeExtendedState.', decoder: 'decodeExtendedState'),
  'BDEx': ViBlockInfo('BDEx', 'Block-diagram extended', _un, _cf, 'u32be flag words (4-32 B dominate); bit meanings not yet decoded. See decodeExtendedState.', decoder: 'decodeExtendedState'),
  ...
  'PRT ': ViBlockInfo('PRT ', 'Print settings', _un, _cf, 'Fixed 128 B (rarely 132/136) print record: version byte 0x01 @4; all-zero in the default form (3737/3840); field semantics not yet decoded. See decodePrintRecord.', decoder: 'decodePrintRecord'),
  'DLDR': ViBlockInfo('DLDR', 'Default-data loader', _un, _cf, 'Fixed 28-byte record, constant across the corpus. See decodeDldrRecord.', decoder: 'decodeDldrRecord'),
  'TRec': ViBlockInfo('TRec', 'Type record', _un, _cf, '13-byte header (semantics not yet decoded) + u16-prefixed text runs (descriptions/tips). See decodeTextRecord.', decoder: 'decodeTextRecord'),
  ...
  'BKMK': ViBlockInfo('BKMK', 'Bookmarks', _un, _lk, 'Bookmark list; an 8-byte empty record when there are none (743/988), larger with bookmark text.', decoder: 'decodeBookmarkList'),
