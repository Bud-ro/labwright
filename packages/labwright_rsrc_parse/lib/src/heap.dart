...
enum HeapAttribute {
  ...
  /// both validity-gated in [decodeHeapAttr]): the `C6 6C FF` blob is help/
  /// description text (only ~71% of bodies are mostly-printable, so the gate keeps
  /// the genuine ones — VI/control descriptions, occasionally a version number —
  /// and frames binary ones), and the `C6 6C <u8 len>` form carries a
  /// `<u32 strlen><ascii>` string — CLF/library and format *tokens* like
  /// "ps2000aRunStreaming", "%f", "#" (see [_u32StringIds]; ~50% validate). NOTE:
  /// the `<u8len>` tokens are NOT help text — callers must not label them "help".
  /// The `C6 6C 08` form is ambiguous and left framed.
...
/// `0x6C` uses it for CLF/library and format tokens ("ps2000aRunStreaming", "%f").
/// Only ~50% of `0x6c <u8len>` records match cleanly, so [decodeHeapAttr]
/// per-record-validates (strlen fits, not a big-slack 1-char false positive, fully
/// printable) and leaves the rest framed — never fabricating a string.
...
HeapAttr? decodeHeapAttr(Uint8List body, int offset) {
  ...
  if (op == 0xc6 && offset + 5 <= body.length && body[offset + 2] == 0xff) {
    ...
    if (end > body.length || len < 4) return null;
  ...
  if (lo == 4 || lo == 5 || lo == 6) {
    ...
    return HeapAttr(attribute: HeapAttribute.fromId(id), id: id, width: width, value: value, length: 2 + valueBytes);
  }
