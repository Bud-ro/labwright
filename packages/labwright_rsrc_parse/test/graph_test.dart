...
void main() {
  test('bracket tree: parent/child nesting + absolute coordinates', () {
    final records = <int>[
      ...
      ...open(0x50, 2, tag: 0x1a), ...bounds(10, 20, 30, 40), ...caption('Trigger'),
      ...close(0x1a),
      ...close(),
    ];
    ...
    expect([child.absBounds!.top, child.absBounds!.left, child.absBounds!.bottom, child.absBounds!.right],
        [10, 20, 30, 40], reason: 'abs = parent origin (0,0) + local bounds');
    expect(d.children(1).map((o) => o.oid), [2]);
  });

  test('absolute coordinates compose down the object-ancestor chain', () {
    final records = <int>[
      ...open(0x7e, 1), ...bounds(100, 200, 900, 900),
      ...open(0x53, 2, tag: 0x1a), ...bounds(5, 5, 50, 50),
      ...open(0x50, 3, tag: 0x1b), ...bounds(1, 2, 11, 12),
      ...close(0x1b), ...close(0x1a), ...close(),
    ];
    final body = Uint8List.fromList([0, 0, 0, records.length, ...records]);
    final leaf = buildDiagram(body).byId[3]!;
    expect([leaf.absBounds!.top, leaf.absBounds!.left], [106, 207],
  ...
  test('structure child-membership refs attach to the structure, not terminals', () {
    final records = <int>[
      ...
      0x10, 0x55, 0x01, 0xfb, 0x00, 0x02,
      0x14, 0x19, 0x01, 0xfd, 0x00, 0x09,
      0x14, 0x19, 0x01, 0xfd, 0x00, 0x0a,
      ...close(0x55),
      ...close(),
    ];
    final body = Uint8List.fromList([0, 0, 0, records.length, ...records]);
  ...
  test('the full 0x14 typed-ref family is collected (childRef + memberRef) into the object graph', () {
    final records = <int>[
      ...open(0x53, 1), ...bounds(0, 0, 100, 100),
      0x14, 0x19, 0x01, 0xfd, 0x00, 0x09,
      0x14, 0x4f, 0x01, 0xfd, 0x00, 0x0b,
      0x14, 0x50, 0x01, 0xfd, 0x00, 0x0c,
      0x14, 0x53, 0x01, 0xfd, 0x00, 0x07,
      ...close(),
    ];
    ...
    expect(s.typedRefs.containsKey(HeapRefKind.literal), isFalse,
  ...
  test('classifies kinds and infers type from attached C4 records', () {
    final records = <int>[
      ...open(0x68, 1), ...bounds(0, 0, 17, 17), ...close(),
      ...open(0x12, 2), ...bounds(0, 0, 40, 40), ...close(),
      ...open(0x50, 3), ...bounds(0, 0, 17, 80), ...fmt74('%#_6g'), ...close(),
      ...open(0x50, 4), ...bounds(0, 0, 17, 80), ...fmt74('%04d'), ...close(),
      ...open(0x0d, 5), ...bounds(0, 0, 17, 80), ...enum2e(['Low', 'High']), ...close(),
    ];
  ...
  test('scrolled-cluster control terminals are re-anchored to their viewport', () {
    final records = <int>[
      ...
      ...open(0x11c, 3), ...bounds(10, 5, 160, 130),
      ...open(0x50, 4), ...bounds(-300, 9, -256, 105),
      ...open(0xa, 6), ...bounds(0, 11, 17, 107), ...caption('Start'),
      ...close(),
      ...open(0x4f, 8), ...bounds(5, 10, 17, 30),
      ...close(),
      ...close(),
      ...open(0x50, 5), ...bounds(-262, 9, -218, 105),
      ...close(),
      ...close(),
      ...close(),
      ...open(0x50, 7), ...bounds(300, 300, 320, 350),
      ...close(),
      ...close(),
    ];
    final d = buildDiagram(Uint8List.fromList([0, 0, 0, records.length, ...records]));
  ...
  test('HeapObjectClass catalog: unique codes, round-trip, category agreement', () {
    ...
    for (final k in [0x20, 0x21, 0xcd, 0x14d]) {
      expect(classifyObject(kind: k, termCount: 0), ViObjectKind.structure,
  ...
  test('section-dependent class labels stay honest about what was observed', () {
    ...
    expect(loop, isNot('Loop (while/for)'), reason: 'a section-blind label would mislabel the FP-container occurrences');
  ...
  test('enum/ring items are parsed and propagated up to the enclosing control', () {
    final records = <int>[
      ...
      ...open(0x57, 2, tag: 0x1a), ...bounds(10, 10, 30, 110),
      ...open(0x0d, 3, tag: 0x1b), ...bounds(12, 12, 28, 100), ...enum2e(['Low', 'Med', 'High']),
      ...close(0x1b),
      ...close(0x1a),
      ...close(),
    ];
  ...
  test('graph plot names (C4 27) attach to the 0x5E graph object', () {
    ...
    final records = <int>[
      ...open(0x7e, 1), ...bounds(0, 0, 400, 400),
      ...open(0x5e, 2, tag: 0x1a), ...bounds(10, 10, 200, 300),
  ...
  test('enum item parsing rejects the WHOLE table on overrun or non-printable bytes', () {
    ...
    expect(nonPrintable.byId[2]!.items, isEmpty, reason: 'an embedded non-printable byte (0x00) rejects the whole table');
  });
  ...
  test('control range (0x20/0x21) + help (0x6C FF) collected ONLY on controls, not decorations', () {
    ...
    final records = <int>[
      ...
      ...f64rec(0x20, -5.0), ...f64rec(0x21, 10.0),
      ...c6blob(0x6c, 'a tooltip'),
      ...close(0x1a),
      ...open(0x8f, 3, tag: 0x1b), ...bounds(0, 0, 10, 10),
      ...f64rec(0x20, 1.0), ...f64rec(0x21, -1.0),
      ...close(0x1b),
      ...close(),
    ];
    final d = buildDiagram(Uint8List.fromList([0, 0, 0, records.length, ...records]));
  ...
  test('help propagation never overwrites an ancestor that already carries its own help (??=)', () {
    ...
    final d = buildDiagram(Uint8List.fromList([0, 0, 0, records.length, ...records]));
    expect(d.byId[1]!.helpText, 'own help', reason: "first-wins (??=): the control's own help is preserved over a child's");
  });
  ...
  test('structural node fallback: an unknown drawable kind under 0x1b with only 0x15 children -> node', () {
    final records = <int>[
      ...
      ...open(0x150, 3, tag: 0x1b), ...bounds(10, 10, 42, 42),
  ...
  test('a BD node inherits its name from its child 0xa caption (for details/tooltip)', () {
    final records = <int>[
      ...open(0x7e, 1), ...bounds(0, 0, 400, 400),
      ...open(0x2f, 2, tag: 0x1a), ...bounds(10, 10, 42, 42),
      ...open(0xa, 3, tag: 0x1b),
      ...caption('Build Array'),
      ...close(0x1b),
      ...close(0x1a),
  ...
  test('buildDiagram terminates on a parentOid cycle (reanchorViewport guard)', () {
    final records = <int>[
      ...
      ...open(0x50, 5, tag: 0x1d), ...bounds(10, 10, 30, 30),
      ...close(0x1d),
      ...close(0x1c),
      ...close(0x1b),
      ...close(0x1a),
  ...
  test('buildDiagram terminates on a duplicate-oid control under a viewport (no infinite loop)', () {
    final records = <int>[
      ...
      ...open(0x11c, 1, tag: 0x1a), ...bounds(10, 10, 200, 200),
      ...open(0x50, 7, tag: 0x1b), ...bounds(-300, 5, -283, 90),
      ...open(0x50, 7, tag: 0x1c), ...bounds(0, 5, 17, 90),
      ...close(0x1c),
      ...close(0x1b),
      ...close(0x1a),
      ...close(),
