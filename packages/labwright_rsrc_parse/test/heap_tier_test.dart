...
HeapDecodeTier tier(List<int> bytes) => heapDecodeTier(Uint8List.fromList(bytes), 0, bytes[0], 'BDHb');
...
void main() {
  ...
  test('semantic: newest decoded forms (0x31 string, 0x6c help blob, 0x20/0x21/0x22 f64, colour)', () {
    ...
    expect(
      tier([0xc5, 0x20, 0x08, 0xbf, 0xf0, 0, 0, 0, 0, 0, 0]),
      HeapDecodeTier.semantic,
      reason: '0x20 in the C5 …08 f64 form = control min',
    );
    expect(
      tier([0xc5, 0x22, 0x08, 0, 0, 0, 0, 0, 0, 0, 0]),
      HeapDecodeTier.semantic,
      reason: '0x22 in the C5 …08 f64 form = control default',
    );
    expect(
      tier([0x84, 0x20, 0xff, 0x10, 0x10, 0x10]),
      HeapDecodeTier.semantic,
