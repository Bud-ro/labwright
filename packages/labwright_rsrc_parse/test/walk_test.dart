...
void main() {
  test('recordSkip frames the known record families', () {
    ...
    expect(recordSkip(b([0x14, 0x19, 0x01, 0xfd, 0x80, 0x00, 0x00, 0x00, 0x84, 0x6f]), 0), 10,
        reason: 'FD value-escape (fd 80 00 <u32>) makes a 10-byte record, not a hardcoded 6 (heap record-size desync fix)');
  ...
  test('walkHeapBody walks a well-formed body to exact EOF', () {
    ...
    expect(w.coverage, 1.0,
        reason: 'walker covers every record; the leading [u32 contentLen] header value is ignored');
  ...
  test('decodeHeapPropertyToken decodes tagged-list values and bare selectors', () {
    ...
    expect(esc!.value, 0x000136de,
        reason: 'FD 7-byte escape (fd 80 00 <u32>): value is the u32, not the 0x8000 bytes');
    ...
    expect(isTypeDescriptorToken(0x04), isTrue,
        reason: '0x04 fragments are type-descriptor grammar, not properties');
  ...
  test('HeapRefKind: subop maps to relationship; 0x53 is a literal, not a ref', () {
    ...
    expect(HeapRefKind.fromSubop(0x34), HeapRefKind.objectRef,
