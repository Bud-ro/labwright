...
- `snapshot.json` — every aggregate corpus metric the corpus tests measure
  (recovery censuses, coverage node counts, writer-scoreboard byte totals,
  exporter gate counts), regenerated by `tool/snapshot.dart` and asserted with
  **exact equality** by `test/snapshot_check.dart`. **Not hand-edited** — any
  change, up or down, must arrive as a reviewed regeneration diff.
