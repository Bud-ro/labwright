...
SeqProjectExport exportSeqProjectToLabwright(Map<String, SeqFile> byPath) {
  ...
  files['lw_runtime.dart'] = [
    ...
    "/// module, mirroring the engine's scoping.",
    'final dynamic stationGlobals = ts.PropObj();',
    '',
  ].join('\n');
...
class _DartExporter {
  ...
  String export() {
    ...
    // A file with no sequences registers no tests — nothing references
    // `lw`, and the import would be the only analyzer noise in the file.
    const lwImport = "import 'package:labwright/labwright.dart' as lw;\n";
    if (asTest &&
        !RegExp(r'(?<![\w\$.])lw\.')
            .hasMatch(source.replaceFirst(lwImport, ''))) {
      source = source.replaceFirst(lwImport, '');
    }
    return source;
  }
  ...
  void _emitHeader() {
    _out
      ...
      ..writeln('// are kept verbatim in ts.eval(...) calls. Nothing is '
          'fabricated: unexportable')
      ..writeln('// steps remain as ordered comments. The ts.* shims '
          '(package:labwright/shims.dart)')
      ..writeln('// host the engine built-ins and the PropObj '
  ...
  String _arrayInit(SeqVariable local) =>
      _listInit(local.raw.array ?? const [], {});
  ...
  void _emitRuntime() {
    // Engine STATE only — the built-in helpers live in
    // package:labwright/shims.dart, so generated files carry no per-file
    // boilerplate. FileGlobals starts from the file's own declared
    // defaults (real data, structure preserved); StationGlobals is
    // station-level shared state; RunState/Step are the ENGINE's
    // execution objects — null placeholders a real host can back.
    final defaults = file.data.prop('FileGlobalDefaults');
    final stationLine = registerName != null
        ? '// Station-wide state (StationGlobals) lives in lw_runtime.dart.'
        : 'final dynamic stationGlobals = ts.PropObj();';
    _out.writeln('''
// ── engine state ────────────────────────────────────────────────────────────

final dynamic fileGlobals =
    ${defaults == null ? 'ts.PropObj()' : _propObjInit(defaults, {})};
$stationLine
...
final dynamic step = null;''');
  ...
  String _propValue(SeqProperty c, Set<String> seenTypes) {
    final elements = c.array;
    if (elements != null) return _listInit(elements, seenTypes);
  ...
  /// A declared array's initializer; long uniform SCALAR runs compress to
  /// List.filled (a 1000-slot pre-sized array must not be 1000 literals —
  /// containers stay per-element, filled() would alias one object).
  String _listInit(List<SeqProperty> elements, Set<String> seenTypes) {
