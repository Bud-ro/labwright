...
void main() {
  ...
  group('tdmsToCsv edge cases', () {
    test('header:false omits the header row', () {
      final bytes = (TdmsWriter()..writeSegment([TdmsChannel(group: 'G', name: 'x', data: const [1.5, 2.5])])).toBytes();
    ...
    test('a file with no numeric channels yields empty CSV', () {
      final bytes = (TdmsWriter()
            ..writeSegment([TdmsChannel(group: 'G', name: 'note', data: const [], properties: {'value': 'hi'})]))
          .toBytes();
  ...
  test('csv -> tdms -> csv -> tdms is numerically stable (column data preserved)', () {
    ...
    List<List<double>> cols(String c) {
      ...
      return [for (final g in f.groups) for (final ch in g.channels) ch.data];
    }
