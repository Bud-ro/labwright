...
void main() {
  ...
  test('quotes header names containing the delimiter', () {
    final bytes = (TdmsWriter()..writeSegment([TdmsChannel(group: 'a,b', name: 'v', data: const [1.0])])).toBytes();
    expect(tdmsToCsv(bytes).split('\n').first, '"a,b/v"');
  });

  test('honors a custom delimiter and header:false', () {
    final bytes = (TdmsWriter()..writeSegment([TdmsChannel(group: 'M', name: 'v', data: const [1.0, 2.0])])).toBytes();
